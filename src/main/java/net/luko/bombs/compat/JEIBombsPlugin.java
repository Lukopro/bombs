    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        for(DemolitionUpgradeRecipe recipe : upgradeRecipes){
            for(ItemStack bomb : bombInputs){
                ...
                DemolitionUpgradeRecipeInput input = new DemolitionUpgradeRecipeInput(
                        bomb, recipe.inputUpgrade().getItems()[0]
                );

                if(recipe.matches(input, Minecraft.getInstance().level)){
                    ItemStack output = recipe.assemble(input, Minecraft.getInstance().level.registryAccess());
                    if(!output.isEmpty()){
                        upgradeDisplayRecipes.add(new UpgradeRecipeDisplay(
                                bomb.copy(), recipe.inputUpgrade(), output.copy()
                        ));
                    }
                }
            }
